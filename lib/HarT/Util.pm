...
has 'harvest' => (
	is => 'rw',
	isa => 'WebService::Harvest'
);

sub BUILD {
	my $self = shift @_;
	$self->harvest(new WebService::Harvest(config => $self->config));
}
...
sub report {
	...
	my $daily_std = $self->config->{daily_standard_hours} || 7.84615384615;
	my $daily_target = $self->config->{daily_target_hours} || 8.5;
	...
	my $entries = $self->harvest->getEntries($s_date, $e_date);
